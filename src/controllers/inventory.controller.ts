...
export class InventoryController {
  ...
  getItemById = async (req: Request, res: Response): Promise<void> => {
    const itemId = safeInt(req.params.id); // Updated to use safeInt
    if (isNaN(itemId)) {
  ...
  addItem = async (req: Request, res: Response): Promise<void> => {
    ...
    const parsedQuantity = safeInt(quantity); // Updated to use safeInt
  ...
  getTransactionsByItemId = async (req: Request, res: Response): Promise<void> => {
    const itemId = safeInt(req.params.itemId); // Updated to use safeInt
    if (isNaN(itemId)) {
      res.status(400).json({ error: 'Invalid item ID.' });
      return;
    }
